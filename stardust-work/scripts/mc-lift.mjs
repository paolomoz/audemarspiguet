// Masterclass-page lift for replica recreation (archetype G): dwell-hydrated
// DOM snapshot + per-module verbatim content manifest + computed-style lift.
// Adapted from home-lift.mjs with a URL parameter and masterclass-specific
// card/search probes.
// Usage: node mc-lift.mjs <url> <width> <out.json> [--dom <out.html>]
...
const PROPS = ['font-family', 'font-size', 'font-weight', 'font-style', 'line-height', 'letter-spacing', 'text-transform', 'color', 'background-color', 'margin-top', 'margin-bottom', 'padding-top', 'padding-bottom', 'padding-left', 'padding-right', 'max-width', 'width', 'height', 'display', 'grid-template-columns', 'gap', 'border-radius', 'border', 'text-align', 'opacity', 'position', 'object-fit', 'aspect-ratio', 'justify-content', 'align-items', 'flex-direction'];
...
const data = await page.evaluate((PROPS) => {
  ...
  const modules = mods.map((el) => {
    ...
    const ctas = [...el.querySelectorAll('a,button')].filter((a) => txt(a) && a.getBoundingClientRect().height > 0).map((a) => ({ tag: a.tagName, text: txt(a).slice(0, 90), href: a.getAttribute('href'), cls: (a.className || '').toString().slice(0, 100), style: cs(a) })).slice(0, 40);
    ...
    const slides = [...el.querySelectorAll('.swiper-slide')].slice(0, 24).map((s) => ({
      cls: (s.firstElementChild?.className || '').toString().slice(0, 120),
      img: imgOf(s.querySelector('img')),
      texts: [...s.querySelectorAll('h3,h4,p,a,span,div,li')].map((n) => ({ tag: n.tagName, cls: (n.className || '').toString().slice(0, 80), t: txt(n)?.slice(0, 160), href: n.tagName === 'A' ? n.getAttribute('href') : null })).filter((x) => x.t && x.t.length < 161).slice(0, 20),
    ...
    // masterclass cards (search results / detail hero card)
    const mcCards = [...el.querySelectorAll('[class*="ap-masterclass-card"]')].filter((c) => !c.closest('.swiper-slide') && c.getBoundingClientRect().height > 8).slice(0, 16).map((c) => ({
      cls: (c.className || '').toString().slice(0, 140),
      rect: { top: Math.round(c.getBoundingClientRect().top + scrollY), h: Math.round(c.getBoundingClientRect().height), w: Math.round(c.getBoundingClientRect().width), left: Math.round(c.getBoundingClientRect().left) },
      style: cs(c),
      parts: [...c.querySelectorAll('*')].filter((n) => {
        const t = txt(n);
        return t && n.children.length === 0 && n.getBoundingClientRect().height > 0;
      }).slice(0, 30).map((n) => ({ tag: n.tagName, cls: (n.className || '').toString().slice(0, 90), t: txt(n).slice(0, 160), style: cs(n) })),
      img: imgOf(c.querySelector('img')),
    }));
    // search/filter chrome
    const searchBits = [...el.querySelectorAll('[class*="masterclass-search"] input, [class*="masterclass-search"] select, [class*="masterclass-search"] [class*="filter"], [class*="masterclass-search"] [class*="dropdown"], [class*="masterclass-search"] label')].slice(0, 30).map((n) => ({
      tag: n.tagName, cls: (n.className || '').toString().slice(0, 120), t: txt(n)?.slice(0, 120), placeholder: n.getAttribute?.('placeholder') || null, style: cs(n),
    }));
    return {
      ...
      headings, paras, ctas, imgs, videos, slides, mcCards, searchBits,
    };
  });
  return {
...
console.log(`[mc-lift] ${width}w: ${data.modules.length} modules, pageH ${data.pageH} -> ${outJson}`);
if (outHtml) {
  const html = await page.content();
  writeFileSync(outHtml, html);
  console.log(`[mc-lift] DOM snapshot -> ${outHtml} (${Math.round(html.length / 1024)}KB)`);
}
await browser.close();
